...
#     del strategy_ret["date"]
    strategy_ret.drop('step',axis=1,inplace=True)
    return pd.Series(strategy_ret["return"].values, index=strategy_ret.index)
...
def get_baseline(ticker, start, end):
    return DP.download_data(
        start_date=start, end_date=end, ticker_list=[ticker], time_interval='1D')
#     ).fetch_data()
